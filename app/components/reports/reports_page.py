...
def date_range_selector() -> rx.Component:
    ...
    return rx.el.div(
        rx.el.h3(
            "Date Range", class_name="text-lg font-semibold text-stone-800 mb-3"
        ),
        rx.el.div(
            rx.foreach(
                ranges,
                lambda r: rx.el.button(
                    ...
                    class_name=rx.cond(
                        ...
                        "px-4 py-2 rounded-lg bg-emerald-500 text-white font-semibold text-sm transition-all",
                        "px-4 py-2 rounded-lg bg-white text-stone-600 font-semibold text-sm border border-stone-200 hover:bg-stone-50 transition-all",
                    ),
                ),
            ),
...
def summary_metric_card(
    ...
    return rx.el.div(
        rx.el.div(
            ...
            class_name="p-3 rounded-full bg-stone-50",
        ),
        rx.el.div(
            rx.el.p(value, class_name="text-2xl font-bold text-stone-800"),
            rx.el.p(label, class_name="text-sm text-stone-500"),
        ),
        class_name="flex items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-stone-100",
    )
            ...
            rx.el.div(
                rx.foreach(
                    ...
                    lambda item: rx.el.div(
                        rx.el.p(item["name"], class_name="font-medium text-stone-700"),
                        rx.el.p(
                            "₹" + item["value"].to_string(),
                            class_name="text-sm text-stone-500",
                        ),
                        class_name="flex items-center justify-between py-2 border-b border-stone-100 last:border-0",
                    ),
                ),
                class_name="w-full",
            ),
            class_name="grid grid-cols-1 lg:grid-cols-2 gap-6 bg-white p-6 rounded-2xl shadow-sm border border-stone-100",
        ),
    )
    ...
    return rx.el.div(
        ...
        rx.el.div(
            rx.el.button(
                ...
                class_name="flex items-center bg-red-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-red-600 transition-all shadow-sm hover:shadow-md",
            ),
            rx.el.button(
                rx.icon("table", class_name="h-5 w-5 mr-2"),
                "Generate Excel Report",
                on_click=ReportsState.generate_excel_report,
                class_name="flex items-center bg-emerald-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-700 transition-all shadow-sm hover:shadow-md",
            ),
