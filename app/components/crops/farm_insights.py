...
def current_weather_card() -> rx.Component:
    return rx.cond(
        ...
        rx.el.div(
            rx.el.h3("Current Weather", class_name="font-semibold text-stone-800"),
            rx.el.div(
                rx.icon(
                ...
                rx.el.div(
                    ...
                    rx.el.p(
                        ...
                        class_name="text-stone-600 font-medium",
                    ),
                ),
            ...
            rx.el.div(
                rx.el.div(
                    rx.icon("wind", class_name="h-4 w-4 text-stone-500"),
                    rx.el.span(
                        f"{CropState.weather_data['current']['wind_speed_10m']} km/h",
                        class_name="text-sm",
                    ),
                    class_name="flex items-center gap-2 text-stone-600",
                ),
                rx.el.div(
                    rx.icon("droplets", class_name="h-4 w-4 text-stone-500"),
                    rx.el.span(
                ...
                rx.el.div(
                    rx.icon("umbrella", class_name="h-4 w-4 text-stone-500"),
                    rx.el.span(
        ...
        rx.el.div(
            class_name="bg-white p-5 rounded-2xl shadow-sm border border-stone-100 animate-pulse h-48"
        ),
    )
...
def daily_forecast_card(day_data: dict[str, str | int]) -> rx.Component:
    return rx.el.div(
        rx.el.p(day_data["day"], class_name="font-semibold text-sm text-stone-600"),
        rx.icon(
        ...
        class_name="text-center p-3 bg-stone-50/50 rounded-xl",
    )
...
def weather_forecast_section() -> rx.Component:
    return rx.cond(
        ...
        rx.el.div(
            class_name="bg-white p-5 rounded-2xl shadow-sm border border-stone-100 col-span-2 animate-pulse h-48"
        ),
    )
...
def suggestion_card(suggestion: dict) -> rx.Component:
    return rx.el.div(
        ...
        rx.el.div(
            rx.el.p(suggestion["title"], class_name="font-semibold text-stone-800"),
            rx.el.p(suggestion["suggestion"], class_name="text-sm text-stone-600"),
        ),
        class_name="flex items-start gap-4 p-4 bg-white rounded-2xl shadow-sm border border-stone-100",
    )
...
def farm_insights_page() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.div(
            rx.el.h2(
                "Farming Suggestions",
                class_name="text-2xl font-bold text-stone-800 my-6",
            ),
            rx.cond(
                CropState.weather_loading,
                rx.el.div(
                    "Loading suggestions...", class_name="text-center text-stone-500"
                ),
                rx.el.div(
